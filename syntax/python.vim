...
let python_highlight_builtin_exceptions = 1
...
if exists("python_highlight_builtin_funcs") && python_highlight_builtin_funcs != 0
  ...
  " NumPy
  syn keyword pythonBuiltinFunc	square dot inv pinv matrix asmatrix zeros randn rand randint
  syn keyword pythonBuiltinFunc	array exp shape sqrt tanh power concatenate cos cosh arccos
  syn keyword pythonBuiltinFunc	diag sin sinh arcsin meshgrid round var std cov eig angle
  syn keyword pythonBuiltinFunc	histogram argmin argmax mean argsort log transpose eye ones
  syn keyword pythonBuiltinFunc	multiply permutation det reshape sort arange seed unique
  syn keyword pythonBuiltinFunc	load loadtxt loads save savetxt savez real complex svd where
  syn keyword pythonBuiltinFunc	tensordot fft2 fft ifft ifft2 conj real squeeze triu tril
  syn keyword pythonBuiltinFunc	copy split sign isnan isinf nan pi e inf prod zeros_like
  syn keyword pythonBuiltinFunc	ones_like seterr multinomial nonzero tile cholesky linspace
  ...
  syn keyword pythonBuiltinFunc	diff delete dirichlet pad norm choice
